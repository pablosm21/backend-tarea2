...
public class RequestBodyExtractor {
    ...
    public GatewayRequest getRequest(ServerWebExchange exchange, DataBuffer buffer) {
        ...
        // If targetMethod is not provided in JSON, use the incoming request method
        if (request.getTargetMethod() == null) {
            request.setTargetMethod(exchange.getRequest().getMethod());
        }

        // If queryParams is not provided, initialize empty
        if (request.getQueryParams() == null) {
            request.setQueryParams(new org.springframework.util.LinkedMultiValueMap<>());
        }

        // If body is not provided, parse the entire JSON as the body
        if (request.getBody() == null) {
            request.setBody(objectMapper.readValue(rawRequest, Object.class));
        }
